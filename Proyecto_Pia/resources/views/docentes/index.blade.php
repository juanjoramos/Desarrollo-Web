        ...
        <table class="min-w-full bg-gray-700 shadow rounded border border-gray-300 text-white">
            ...
            <tbody>
                @foreach ($docentes as $docente)
                    <tr>
                        <td class="border px-4 py-2">{{ $docente->id }}</td>
                        ...
                        <td class="border px-4 py-2 space-x-2">
                            ...
                            <form action="{{ route('docentes.destroy', $docente) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="text-red-500 hover:underline" onclick="return confirm('¿Eliminar este docente?')">Eliminar</button>
                            </form>
                        </td>
                    </tr>
                @endforeach
                ...
                @if ($docentes->isEmpty())
                    <tr>
                        <td colspan="5" class="text-center py-4 text-gray-300">No hay docentes registrados.</td>
                    </tr>
                @endif
            </tbody>
        </table>
